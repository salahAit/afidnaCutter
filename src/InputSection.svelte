<script>
  ...
  async function handleYoutubeFetch() {
    ...
    try {
        const response = await window.electron.invoke('download-youtube', { url: youtubeUrl });
  ...
  async function pollDownloadStatus() {
    const interval = setInterval(async () => {
        try {
            const status = await window.electron.invoke('get-status', appState.sessionId);
            if (status.status === 'downloading') {
                appState.downloadStatus = { status: 'downloading', progress: status.progress };
...
<div class="bg-slate-800 p-6 rounded-2xl mb-4 border border-slate-700 shadow-lg">
  <div class="flex gap-4 mb-4 border-b border-slate-700 pb-2">
    <button 
      class="px-4 py-2 rounded-lg transition-colors {appState.mode === 'youtube' ? 'bg-blue-500/10 text-blue-400 font-bold' : 'text-slate-400 hover:text-slate-200'}"
      onclick={() => switchMode('youtube')}
    >
      رابط يوتيوب
    </button>
    <button 
      class="px-4 py-2 rounded-lg transition-colors {appState.mode === 'upload' ? 'bg-blue-500/10 text-blue-400 font-bold' : 'text-slate-400 hover:text-slate-200'}"
  ...
  {#if appState.mode === 'youtube'}
    <div class="flex gap-2">
      ...
      />
      <button class="bg-blue-500 hover:bg-blue-600 text-white font-semibold px-6 py-3 rounded-lg transition-colors" onclick={handleYoutubeFetch}>جلب</button>
    </div>
  {:else}
    <div>
      <button class="w-full bg-blue-500 hover:bg-blue-600 text-white font-semibold px-6 py-3 rounded-lg transition-colors" onclick={handleFileSelect}>اختر ملف فيديو</button>
    </div>
  {/if}
  ...
  {#if appState.downloadStatus.status !== 'idle'}
    <div class="mt-4">
      <div class="w-full h-1.5 bg-slate-900 rounded-full overflow-hidden">
        <div class="h-full bg-blue-500 transition-all duration-300" style="width: {appState.downloadStatus.progress}%"></div>
      </div>
