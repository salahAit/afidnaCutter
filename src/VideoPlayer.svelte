<script>
  import { appState, resetState, formatTime } from "./lib/state.svelte.js";
...
>
  {#if appState.videoSrc || appState.youtubeMetadata}
    ...
    {#if appState.mode === "youtube" && appState.youtubeMetadata}
      ...
      <!-- YouTube iframe player (only for YouTube URLs) -->
      {#if appState.youtubeMetadata.isYouTube && appState.youtubeMetadata.id}
        <div bind:this={youtubeContainer} class="w-full h-full"></div>
      {:else}
        <!-- Non-YouTube: Show thumbnail with video info -->
        <div
          class="absolute inset-0 flex flex-col items-center justify-center bg-base-300 z-10 p-6"
        >
          {#if appState.youtubeMetadata.thumbnail}
            <img
              src={appState.youtubeMetadata.thumbnail}
              alt="Thumbnail"
              class="max-w-full max-h-[60%] object-contain rounded-lg shadow-xl mb-4"
            />
          {:else}
            <div
              class="w-48 h-48 bg-base-200 rounded-lg flex items-center justify-center mb-4"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
          ...
          <h3
            class="text-lg font-bold text-center text-base-content mb-2 max-w-md"
            dir="auto"
          >
          ...
          <div class="alert alert-warning text-sm max-w-md">
            <svg
              xmlns="http://www.w3.org/2000/svg"
            ...
            <span
              >{i18n.lang === "ar"
                ? "أضف المقاطع يدوياً من الشريط الجانبي ثم اضغط قص"
                : "Add segments manually from the sidebar then click Cut"}</span
            >
          </div>
        </div>
      {/if}
